...
searchInput.addEventListener("input", function () {
  ...
  if (searchTerm.length > 0) {
    ...
    // Xóa kết quả tìm kiếm nếu không có từ khóa nhập vào
    clearSearchResults();
  }
});
...
function displaySearchResults(results) {
  ...
  trackList.innerHTML = ""; // Xóa bỏ các kết quả tìm kiếm trước đó
...
unction handleSearchInputBlur() {
  const searchInput = document.getElementById("search-input");
  if (searchInput.value.trim() === "") { // Kiểm tra xem ô nhập tìm kiếm có trống không
    // Nếu trống, gọi hàm để xóa kết quả tìm kiếm và hiển thị lại danh sách ban đầu
    clearSearchResults();
    fetchInitialTracks(); // Đây là hàm bạn cần tạo để lấy danh sách ban đầu
  }
}

// Thêm sự kiện lắng nghe cho sự kiện blur trên ô nhập tìm kiếm
document.getElementById("search-input").addEventListener("blur", handleSearchInputBlur);
