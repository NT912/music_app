...
function addToPlaylist(trackName, singer, trackPath) {
  ...
  if (!playlist.some((t) => t.trackPath === trackPath)) {
    ...
    fetch("/tracks/add-to-playlist", {
      ...
      .then((data) => {
        console.log(data);
        var songName = getSongNameById(songId); // Hàm để lấy tên bài hát theo ID
        var playlistName = getPlaylistNameById(playlistId); // Hàm để lấy tên danh sách theo ID
        displaySuccessMessage(songName, playlistName);
      })
      .catch((error) => {
