...
function showNotification(message) {
  ...
  }, 5000); // Thời gian hiển thị thông báo (miligiây)
}
...
function displaySuccessMessage(trackName) {
  // Tạo một element div mới cho thông báo
  var successMessage = document.createElement("div");
  successMessage.textContent =
    'Bài hát "' + trackName + '" đã được thêm vào danh sách thành công!';
  successMessage.classList.add("success-message");

  // Thêm thông báo vào phần tử cha (ví dụ: một phần tử có id là 'notification')
  var notificationArea = document.getElementById("notification");
  notificationArea.appendChild(successMessage);

  // Xóa thông báo sau một khoảng thời gian (ví dụ: sau 5 giây)
  setTimeout(function () {
...
function addToPlaylist(trackName, singer, trackPath) {
  ...
  if (!playlist.some((t) => t.trackPath === trackPath)) {
    ...
    fetch("/tracks/add-to-playlist", {
      ...
      .then((data) => {
        console.log(data);
        displaySuccessMessage(trackName);
      })
      .catch((error) => {
