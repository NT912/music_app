...
function addToPlaylist(trackName, singer, trackPath) {
  ...
  if (!playlist.some((t) => t.trackPath === trackPath)) {
    ...
    fetch("/tracks/add-to-playlist", {
      ...
      body: JSON.stringify({ userId: getUserIdFromSession(), track }),
    })
  ...
  results.forEach((result) => {
    ...
    playIcon.onclick = function () {
      addToPlaylistAndPlay(
        result.track_name,
        result.singer,
        `${encodeURIComponent(result.tracks_path.split("/").pop())}`
      );
    };
...
// Thêm bài hát vào danh sách phát của người dùng
function addToPlaylistAndPlay(trackName, singer, trackPath) {
  let track = { trackName, singer, trackPath };
  let userId = req.session.user_id; // Lấy id người dùng từ session
  if (playlist.length === 0) {
    playlist.push(track);
    playAudio(trackName, singer, trackPath);

    // Thêm bài hát vào bảng playlists
    fetch("/tracks/add-to-playlist", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify({ trackName, singer, trackPath, userId }),
    })
      .then((response) => response.json())
      .then((data) => {
        console.log(data);
...
// Lấy danh sách bài hát của người dùng và hiển thị trong sidebar
function fetchUserPlaylist() {
  let userId = req.session.user_id; // Lấy id người dùng từ session
  fetch(`/tracks/user-playlist/${userId}`)
    .then((response) => response.json())
    .then((data) => {
      console.log(data);
      // Hiển thị danh sách bài hát trong sidebar
      displayUserPlaylist(data);
    })
    .catch((error) => {
      console.error("Lỗi khi lấy danh sách bài hát của người dùng:", error);
    });
}
