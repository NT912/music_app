...
function updateRepeatIcon() {
  ...
  repeatIcon.classList.remove("active", "repeat-one");
  if (isRepeat) {
...
seekBar.addEventListener("input", function () {
  // Tính toán thời gian mới dựa trên vị trí của thanh range
  const seekTime = audioPlayer.duration * (seekBar.value / 100);
  // Đặt thời gian phát của audio player
  audioPlayer.currentTime = seekTime;
});

// Cập nhật thanh range khi audio player thay đổi thời gian phát
audioPlayer.addEventListener("timeupdate", function () {
  // Tính toán thời gian hiện tại và thời gian tổng cộng của bài hát
  const currentMinutes = Math.floor(audioPlayer.currentTime / 60);
  const currentSeconds = Math.floor(
    audioPlayer.currentTime - currentMinutes * 60
  );
  const totalMinutes = Math.floor(audioPlayer.duration / 60);
  const totalSeconds = Math.floor(audioPlayer.duration - totalMinutes * 60);

  // Hiển thị thời gian đang phát đến và thời gian tổng cộng của bài hát
