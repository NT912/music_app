...
function showNotification(message) {
  ...
  // Thời gian hiển thị thông báo (miligiây)
  const displayTime = 5000; // 5 giây
  ...
  // Dừng đếm thời gian khi người dùng tương tác
  notificationElement.addEventListener("mouseover", () => {
    clearTimeout(timeout);
  });

  // Tiếp tục đếm thời gian khi người dùng rời chuột
...
addSongForm.onsubmit = function (event) {
  ...
  fetch("/tracks/add-song", {
    ...
    .then((data) => {
      console.log(data);
      showNotification("Bài hát đã được thêm thành công");
...
function deleteTrack(trackId) {
  fetch(`/tracks/delete-song/${trackId}`, {
    ...
    .then((data) => {
      console.log(data);
      showNotification("Bài hát đã được xóa thành công");
