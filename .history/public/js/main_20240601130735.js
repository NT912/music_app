...
function playTrack(trackPath) {
  ...
  // Kiểm tra xem audioPlayer có tồn tại không
  if (audioPlayer) {
    // Thiết lập đường dẫn của bài hát và phát nó
    audioPlayer.src = trackPath;
    audioPlayer.play();

    // Hiển thị thông tin về bài hát đang phát ở footer
    var currentTrackInfo = document.getElementById("current-track-info");
    currentTrackInfo.textContent = trackPath;

    // Thêm bài hát vào danh sách đang phát ở footer
    var playlist = document.getElementById("playlist");
    var newTrackItem = document.createElement("li");
    newTrackItem.textContent = trackPath;
    playlist.appendChild(newTrackItem);
  } else {
    console.error("Không thể tìm thấy đối tượng audio player trong DOM.");
  }
}
