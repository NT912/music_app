...
function addToPlaylist(trackName, singer, trackPath) {
  ...
  if (!playlist.some((t) => t.trackPath === trackPath)) {
    ...
    // Gửi yêu cầu API để lưu track_id vào bảng playlists
...
function fetchPlaylist() {
  ...
  // Gửi yêu cầu API để lấy danh sách bài hát từ bảng playlists dựa trên user_id
  fetch(`/tracks/playlist/${userId}`)
    .then((response) => response.json())
    .then((data) => {
      console.log(data);
...
function displayPlaylist(playlist) {
  ...
  const playlistUl = sidebar.querySelector(".playlist");
  ...
  if (playlist.length > 0) {
    ...
    var li = document.createElement("li");
    li.textContent = "Không có bài hát nào trong danh sách phát";
    playlistUl.appendChild(li);
  }
}
