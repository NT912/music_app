...
function toggleOptionsMenu(element) {
  ...
  // Stop propagation to prevent window click event
  event.stopPropagation();
}

// Đóng menu 3 chấm và sidebar playlist khi click ra ngoài
window.addEventListener("click", function (event) {
  ...
  // Đóng sidebar playlist
  let sidebar = document.getElementById("playlist-sidebar");
  if (
    sidebar.style.width === "250px" &&
    !sidebar.contains(event.target) &&
    event.target.id !== "playlist-icon"
  ) {
    sidebar.style.width = "0px";
  }

  // Ẩn popup thêm bài hát nếu click ra ngoài
...
window.onclick = function (event) {
  ...
  // Đóng sidebar playlist khi click ra ngoài
  let sidebar = document.getElementById("playlist-sidebar");
  if (
    sidebar.style.width === "250px" &&
    !sidebar.contains(event.target) &&
