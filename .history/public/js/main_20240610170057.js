...
searchInput.addEventListener("input", function () {
  ...
  if (searchTerm.length > 0) {
    fetch(`/tracks/search?q=${searchTerm}`)
      .then((response) => response.json())
      .then((data) => {
        console.log("Search results:", data); // Log dữ liệu nhận được để kiểm tra
...
function displaySearchResults(results) {
  ...
  results.forEach((result) => {
    ...
    playIcon.onclick = function () {
      const trackPath = result.tracks_path;
      console.log("Playing track:", trackPath); // Log đường dẫn của file nhạc để kiểm tra
      addToPlaylistAndPlay(result.track_name, result.singer, trackPath);
    };
