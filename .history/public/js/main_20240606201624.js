...
document.addEventListener("DOMContentLoaded", function () {
  ...
  searchForm.addEventListener("submit", function (event) {
    ...
    fetch(`/search?query=${encodeURIComponent(query)}`, {
      ...
      .then((response) => response.json())
      .then((data) => {
        // Xử lý kết quả trả về từ server, ví dụ: render danh sách bài hát tìm được
  ...
  function renderSearchResults(tracks) {
    ...
    trackList.innerHTML = ""; // Xóa bất kỳ nội dung nào đã tồn tại trong phần hiển thị danh sách bài hát
    ...
    tracks.forEach((track) => {
      ...
      const trackSinger = document.createElement("p");
      trackSinger.textContent = track.singer;

      const playIcon = document.createElement("i");
      playIcon.classList.add("fas", "fa-play-circle", "play-icon");
      playIcon.onclick = function () {
        addToPlaylistAndPlay(track.trackName, track.singer, track.trackPath);
      };
      ...
      addToPlaylistLink.textContent = "Thêm vào danh sách phát";
      addToPlaylistLink.onclick = function () {
        addToPlaylist(track.trackName, track.singer, track.trackPath);
      };
      optionsMenu.appendChild(addToPlaylistLink);

      trackCard.appendChild(trackTitle);
      trackCard.appendChild(trackSinger);
