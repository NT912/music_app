...
document.addEventListener("DOMContentLoaded", function () {
  // Lắng nghe sự kiện chuột phải cho mỗi thẻ li trong sidebar
  document.querySelectorAll(".sidebar li").forEach(function (li) {
    li.addEventListener("contextmenu", function (event) {
      event.preventDefault(); // Ngăn chặn hành động mặc định của chuột phải
      const trackId = li.dataset.trackId; // Lấy id của bài hát từ thuộc tính data-track-id
      const confirmation = confirm("Bạn có chắc chắn muốn xóa bài hát này?");

      if (confirmation) {
        // Gửi yêu cầu xóa bài hát đến server
        fetch(`/tracks/remove/${trackId}`, {
          method: "DELETE",
        })
          .then((response) => response.json())
          .then((data) => {
            if (data.success) {
              // Xóa thẻ li khỏi sidebar nếu xóa thành công
              li.remove();
              alert(data.message);
            } else {
          ...
          .catch((error) => {
            console.error("Lỗi khi gửi yêu cầu xóa bài hát:", error);
            alert("Đã xảy ra lỗi khi xóa bài hát");
          });
      }
    });
