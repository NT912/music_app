...
exports.getUserPlaylist = (req, res) => {
  ...
  Playlist.getUserPlaylist(userId, (err, playlist) => {
    if (err) {
      console.error("Lỗi khi lấy danh sách phát của người dùng:", err);
      res
        .status(500)
        .json({
          error: "Đã xảy ra lỗi khi lấy danh sách phát của người dùng.",
        });
    } else {
      res.json(playlist);
    }
