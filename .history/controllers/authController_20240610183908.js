...
function savePlaylistAfterLogin(userId) {
  Playlist.addToPlaylist(userId, trackId, (err, playlistId) => {
    if (err) {
      console.error("Lỗi khi lưu danh sách phát:", err);
    } else {
      console.log("Danh sách phát đã được lưu thành công:", playlistId);
    }
  });
}
...
function handleSuccessfulLogin(userId) {
  ...
  // Lưu danh sách phát sau khi đăng nhập
  savePlaylistAfterLogin(userId);
